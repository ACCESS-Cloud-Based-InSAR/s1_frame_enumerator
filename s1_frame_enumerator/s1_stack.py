...
def filter_s1_stack_by_geometric_coverage_per_frame(df_stack: gpd.GeoDataFrame,
                                                    frames: List[S1Frame],
                                                    minimum_coverage_ratio_per_frame: float = .5
                                                    ) -> gpd.GeoDataFrame:
    df_stack_one_pass = df_stack.dissolve(by='repeat_pass_timestamp',
                                          aggfunc={'start_time': 'min'},
                                          as_index=False)
...
def get_s1_stack(frames: List[S1Frame],
                 ...
                 minimum_coverage_ratio_per_frame: float = .25,
    ...
    if minimum_coverage_ratio_per_pass:
        ...
        df = filter_s1_stack_by_geometric_coverage_per_pass(df, frames,
                                                            minimum_coverage_per_pass_ratio=ratio)
        if df.empty:
    ...
    if minimum_coverage_ratio_per_frame:
        ...
        df = filter_s1_stack_by_geometric_coverage_per_frame(df,
                                                             frames,
                                                             minimum_coverage_ratio_per_frame=ratio)
        if df.empty:
        ...
        if minimum_coverage_ratio_per_frame < MINIMUM_PER_FRAME_RATIO:
            warn(f'Requesting per frame coverage below {MINIMUM_PER_FRAME_RATIO}%; '
                 'ISCE2 requires minimum number of bursts',
                 category=UserWarning)
    else:
        warn('No per frame check was performed so enumeration should be done by date',
             category=UserWarning)
